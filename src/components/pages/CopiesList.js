...
class CopiesList extends Component {
    constructor(props) {
        super(props);
        this.state ={
            error:null,
            ...
            columns: [{
                ...
                Header: 'Menu',
                Cell: table => {
                    let path = '/copies/edit/' + table.row.original.id;
                    return (
    ...
    deleteItem = async (event) => {
        ...
        await ApiHelper.deleteCopy(idToDel).then(this.fetch).catch((e)=>alert(e)).finally(()=>this.setState({loading:false}))
    };


    fetch = async() => {
        return ApiHelper.getCopies().then( response =>{
